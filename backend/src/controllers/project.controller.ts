...
const generateTaskCode = async (projectId: number): Promise<string> => {
  ...
  const prefix = project.projectCode ? `${project.projectCode}-` : `T${projectId}-`;
...
export const getProjectHierarchy = async (req: Request, res: Response): Promise<void> => {
  try {
    const userId = req.user?.id;
    const userRole = req.user?.role;
    const userDepartmentId = req.user?.departmentId;

    // For employees, get project IDs where they have assigned tasks
    let projectIdsWithMyTasks: number[] = [];
    if (userRole === 'employee' && userId) {
      ...
      projectIdsWithMyTasks = [...new Set(tasksAssignedToMe.map((t: any) => t.projectId))];
    }

    // Recursive function to build tree
    const buildTree = async (parentId: number | null, depth: number = 0): Promise<any[]> => {
      if (depth > 10) return []; // Prevent infinite recursion

      let projects: Project[];

      const baseWhere: any = parentId === null ? { parentId: { [Op.is]: null } } : { parentId };
      ...
      for (const project of projects) {
        ...
        const children = await buildTree(project.id, depth + 1);
    ...
    const hierarchy = await buildTree(null);
    res.json(hierarchy);
  } catch (error) {
    logger.error('Error fetching project hierarchy:', error);
...
export const createProjectWithCode = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (parentId) {
      const parent = await Project.findByPk(parentId);
      if (!parent) {
        res.status(400).json({ message: 'Parent project not found' });
        return;
      }
    }

    // Generate or use provided project code
    const finalCode = projectCode || await generateProjectCode();

    // Check if code already exists
    const existingProject = await Project.findOne({ where: { projectCode: finalCode } });
    if (existingProject) {
    ...
    logger.info(`Project created with code: ${project.id} - ${project.name} (${finalCode})`);
    res.status(201).json(createdProject);
  } catch (error) {
    logger.error('Error creating project:', error);
    res.status(500).json({ message: 'Failed to create project' });
...
export const moveProject = async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    ...
    if (newParentId !== null && newParentId !== undefined) {
      const parent = await Project.findByPk(newParentId);
      if (!parent) {
        res.status(400).json({ message: 'New parent project not found' });
        return;
      }

      // Prevent circular reference
      if (newParentId === project.id) {
        res.status(400).json({ message: 'Cannot move project to itself' });
        return;
      }

      // Check if newParentId is a descendant of the project
      const checkDescendant = async (parentId: number): Promise<boolean> => {
        const children = await Project.findAll({ where: { parentId } });
      ...
      if (await checkDescendant(project.id)) {
        res.status(400).json({ message: 'Cannot move project to its own descendant' });
        return;
      }
    }

    await project.update({ parentId: newParentId || null });
    ...
    logger.info(`Project moved: ${id} to parent ${newParentId || 'root'}`);
    res.json(updatedProject);
  } catch (error) {
